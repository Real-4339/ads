...
class DWT_MLEAD:
    def __init__(
        ...
        self.window_sizes = np.array(
            [
                max(2, self.max_level - l - self.start_level + 1)
    ...
    def detect(self) -> np.ndarray:
        levels, approx_coefs, detail_coefs = multilevel_dwt(
            self.data,
            wavelet="haar",
            mode="periodic",
        ...
        coef_anomaly_counts = []
        for x, level in zip(
            combine_alternating(detail_coefs, approx_coefs), levels.repeat(2, axis=0)
        ):
    ...
    def _estimate_gaussian_likelihoods(
        self, level: float, x_view: np.ndarray
    ) -> np.ndarray:
    ...
    def find_cluster_anomalies(
        self,
        point_anomaly_scores: np.ndarray,
        d_max: float,
        anomaly_counter_threshold: float,
    ) -> list[AnomalyCluster]:
    ...
    def plot(
        self,
        point_anomaly_scores: Optional[np.ndarray] = None,
        ...
        if coefs:
            if (
                self.coefs_levels_ is None
                or self.coefs_approx_ is None
                or self.coefs_detail_ is None
                or self.coefs_scores_ is None
            ):
                import sys

                print(
                    f"Cannot plot coefs, because they were not tracked! "
                    f"Use `track_coefs=True` to enable plotting coefs.",
                    file=sys.stderr,
                )
            else:
                for i, d, a, s in zip(
                    ...
                    df[f"DetailCoef Level {i}"] = d.repeat(self.m // len(d), axis=0)[
                        : self.n
                    ]
                    df[f"ApproxCoef Level {i}"] = a.repeat(self.m // len(a), axis=0)[
                        : self.n
                    ]
                    df[f"Coef Score Level {i}"] = s.repeat(self.m // len(s), axis=0)[
                        : self.n
                    ]
        ...
        if coefs and self.coefs_levels_ is not None:
            ...
            for i in self.coefs_levels_:
                ...
                scale = np.max(
                    [df[f"DetailCoef Level {i}"], df[f"ApproxCoef Level {i}"]]
                ) / np.max(df[f"Coef Score Level {i}"])
                axs[i - self.start_level + 1].plot(
                    df[f"Coef Score Level {i}"] * scale, label=f"Coef Score Level {i}"
                )
